...
function MiniStat({ label, value, icon: Icon, iconColor, iconBg, valueColor = 'text-slate-800' }: MiniStatProps) {
  return (
    <div className="flex items-center gap-3 p-3.5 bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
      ...
      <div className="min-w-0">
        ...
        <p className={`text-base font-extrabold tabular-nums tracking-tight leading-none ${valueColor}`}>
          {value}
        </p>
      </div>
    </div>
        ...
        <div className="p-4 md:p-5 border-b border-slate-100 bg-slate-50/50">
          ...
          {hasActiveFilters && (
            <p className="text-[11px] text-slate-500 mt-2.5">
              ...
              <span className="font-bold text-slate-700">{expenseTransactions.length}</span> expenses
            </p>
          )}
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredExpenses.length === 0 ? (
                ...
                paginatedExpenses.map((t) => {
                  ...
                  const methodCfg = METHOD_CONFIG[t.method];
