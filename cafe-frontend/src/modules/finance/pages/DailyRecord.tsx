...
export default function DailyRecord() {
  ...
  // ── Enrich records ─────────────────────────────────────────────────────────
  const enrichedRecords = useMemo<EnrichedRecord[]>(() => dailyRecords, [dailyRecords]);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95">
      ...
      {summaryStats && (() => {
        ...
        const cashPct = totalRevForBar > 0 ? Math.round((totals.cashSales / totalRevForBar) * 100) : 0;
        const bkashPct = totalRevForBar > 0 ? Math.round((totals.bkashSales / totalRevForBar) * 100) : 0;
        const bankPct = 100 - cashPct - bkashPct;
        const profitRate = summaryStats.days > 0
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              {totalRevForBar > 0 && (
                <div className="flex-1 min-w-0 bg-white border border-slate-100 rounded-xl p-3.5 shadow-sm">
                  ...
                  <div className="flex h-2 rounded-full overflow-hidden gap-px mb-2">
                    <div className="bg-emerald-400 transition-all" style={{ width: `${cashPct}%` }} />
                    <div className="bg-fuchsia-400 transition-all" style={{ width: `${bkashPct}%` }} />
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        Adjust the date range to see daily records
                      </p>
                    </div>
                  </div>
                </td>
