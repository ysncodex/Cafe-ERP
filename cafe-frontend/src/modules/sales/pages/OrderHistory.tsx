...
import { transactionToOrder } from '../utils/orderUtils';
...
> = {
  pending:   { label: 'Pending',    bg: 'bg-amber-50',   text: 'text-amber-700',   icon: Clock,        dot: 'bg-amber-500'   },
  completed: { label: 'Completed',  bg: 'bg-emerald-50', text: 'text-emerald-700', icon: CheckCircle2, dot: 'bg-emerald-500' },
  refunded:  { label: 'Refunded',   bg: 'bg-amber-50',   text: 'text-amber-700',   icon: RotateCcw,    dot: 'bg-amber-500'   },
  voided:    { label: 'Voided',     bg: 'bg-red-50',     text: 'text-red-600',     icon: XCircle,      dot: 'bg-red-500'     },
};
...
> = {
  in_store:  { label: 'In-Store',  icon: Store,       badgeClass: 'bg-emerald-50 text-emerald-700 border border-emerald-200' },
  foodpanda: { label: 'Foodpanda', icon: ShoppingBag, badgeClass: 'bg-orange-50 text-orange-700 border border-orange-200'   },
  foodi:     { label: 'Foodi',     icon: Utensils,    badgeClass: 'bg-violet-50 text-violet-700 border border-violet-200'   },
};
...
function findTopOrderId(rows: Transaction[]): string | null {
  ...
  for (const t of completed) {
    if (
      ...
      || (t.amount === top.amount && itemCount(t) === itemCount(top) && new Date(t.date).getTime() > new Date(top.date).getTime())
    ) {
      top = t;
    }
  }
...
function PendingPaymentModal({
  ...
  tx: Transaction;
  onClose: () => void;
  onComplete: (updated: Transaction) => void;
}) {
  const baseOrder = useMemo(() => transactionToOrder(tx), [tx]);
  const [customerPaidStr, setCustomerPaidStr] = useState(String(tx.amount));
  const [paymentMethod, setPaymentMethod] = useState<PaymentMethod>(tx.method);
    ...
    return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
          <p className="text-sm text-slate-600 mb-4">This order cannot be opened for payment — line items are missing.</p>
          <button onClick={onClose} className="px-4 py-2 rounded-xl bg-slate-800 text-white text-sm font-semibold">Close</button>
        </div>
      </div>
    );
  }
  ...
  const finalize = async (withPrint: boolean) => {
    ...
    onComplete({ ...tx, receiptStatus: 'completed', method: paymentMethod });
    toast.success(`Payment received · ${orderLabel(tx)} completed`);
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl w-full max-w-sm flex flex-col overflow-hidden">
        <div className="bg-gradient-to-br from-amber-500 to-amber-600 px-5 pt-5 pb-4 text-white shrink-0">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs opacity-90 mt-0.5">{orderLabel(tx)} · ৳{tx.amount}</p>
            </div>
        ...
        <div className="p-4 space-y-3">
          <div>
            <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Payment Method</p>
            ...
            <div className="grid grid-cols-4 gap-1.5">
              {quickAmounts.map((amt) => (
                <button
                  key={amt}
                  type="button"
                  onClick={() => setCustomerPaidStr(String(amt))}
          ...
          <div>
            <label className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Customer Paid</label>
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) => setCustomerPaidStr(e.target.value)}
                className={`w-full pl-8 pr-4 py-3 rounded-xl border text-lg font-bold outline-none ${
                  paymentOk ? 'border-emerald-300 bg-emerald-50 text-emerald-800' : 'border-red-300 bg-red-50 text-red-700'
                }`}
              />
            </div>
          </div>
          {paymentOk && paid > 0 && (
            <div className={`rounded-xl p-3 flex items-center gap-3 ${change === 0 ? 'bg-emerald-50 border border-emerald-200' : 'bg-blue-50 border border-blue-200'}`}>
              <ArrowLeftRight size={18} className={change === 0 ? 'text-emerald-600' : 'text-blue-600'} />
              <div>
                <p className="text-[11px] font-semibold uppercase text-slate-500">{change === 0 ? 'Exact Payment' : 'Change'}</p>
                <p className="text-xl font-black">{change === 0 ? 'No Change' : `৳${change}`}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-sm bg-white shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 text-white px-5 pt-5 pb-6 shrink-0">
          <div className="flex items-start justify-between mb-4">
            ...
            <button onClick={onClose} className="w-7 h-7 rounded-lg bg-white/15 hover:bg-white/25 flex items-center justify-center transition-colors">
              <X size={15} />
            </button>
          </div>
            ...
            <div className="bg-white/10 rounded-xl p-2.5 text-center">
              ...
              <p className={`text-[11px] font-bold mt-0.5 ${
                status === 'completed' ? 'text-emerald-300'
                  ...
                  : status === 'refunded' ? 'text-amber-300'
                  : 'text-red-300'
              }`}>{STATUS_CONFIG[status].label}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {/* Order Meta */}
          <section>
            <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Order Info</h3>
            <div className="bg-slate-50 rounded-xl divide-y divide-slate-100">
              ...
              {channel && (
                <div className="flex justify-between px-3 py-2 text-sm items-center">
                  ...
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold ${channel.badgeClass}`}>
              ...
              <div className="flex justify-between px-3 py-2 text-sm items-center">
                ...
                <span className={`inline-flex items-center gap-1.5 font-semibold ${method.color}`}>
                  <MethodIcon size={13} />
                  {method.label}
                </span>
              </div>
          ...
          {tx.receiptLines && tx.receiptLines.length > 0 && (
            <section>
              <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Order Items</h3>
              <div className="bg-slate-50 rounded-xl overflow-hidden">
                ...
                {tx.receiptLines.map((line, i) => (
                  <div key={i} className={`grid grid-cols-[1fr_auto_auto] px-3 py-2 border-t border-slate-100 text-sm ${line.isGift ? 'bg-emerald-50/50' : ''}`}>
                    <span className="font-medium text-slate-800 leading-snug pr-2 flex items-center gap-1.5 min-w-0">
                      <span className="truncate">{line.name}</span>
                      {line.isGift && (
                        <span className="shrink-0 text-[9px] font-bold uppercase text-emerald-600 bg-emerald-100 px-1 py-0.5 rounded">Gift</span>
                      )}
                    </span>
                    <span className="text-center w-8 text-slate-500 font-semibold">{line.qty}×</span>
          ...
          {(!tx.receiptLines || tx.receiptLines.length === 0) && (
            <section>
              <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Description</h3>
          ...
          {/* Payment Summary */}
          <section>
            <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Payment Summary</h3>
            <div className="bg-slate-50 rounded-xl divide-y divide-slate-100">
              {tx.receiptLines && tx.receiptLines.length > 0 && (
                <div className="flex justify-between px-3 py-2 text-sm">
                  <span className="text-slate-500">Subtotal</span>
                  <span className="font-medium">৳{tx.receiptLines.reduce((s, r) => s + r.qty * r.unitPrice, 0)}</span>
                </div>
              )}
              ...
              {tx.giftItemCount && tx.giftItemCount > 0 ? (
                <div className="flex justify-between px-3 py-2 text-sm text-emerald-700">
                  <span className="flex items-center gap-1"><Gift size={12} /> Gift Items ({tx.giftItemCount})</span>
        ...
        <div className="border-t border-slate-100 p-4 space-y-2 shrink-0 bg-white">
          {isPending ? (
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold text-sm transition-colors"
            >
              <Printer size={15} />
              Print Receipt
            </button>
...
function DeleteConfirmModal({ tx, onConfirm, onCancel }: { tx: Transaction; onConfirm: () => void; onCancel: () => void }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
...
function SortBtn({
  ...
  return (
    ...
    >
      {label}
      {active
        ? (sortDir === 'asc' ? <ChevronUp size={13} /> : <ChevronDownIcon size={13} />)
...
export default function OrderHistory() {
  const { transactions, updateTransaction, deleteTransaction, itemNames, suppliers } = useERP();
  ...
  const filtered = useMemo(() => {
    ...
    if (dateFrom) rows = rows.filter((t) => new Date(t.date) >= new Date(dateFrom));
    if (dateTo)   rows = rows.filter((t) => new Date(t.date) <= new Date(dateTo + 'T23:59:59'));
  ...
  }, [allSales, search, statusFilter, methodFilter, channelFilter, dateFrom, dateTo, sortField, sortDir]);
  ...
  // ── KPIs ──
  const kpis = useMemo(() => {
    const active = allSales.filter((t) => statusOf(t) === 'completed');
    const pendingCount = allSales.filter((t) => statusOf(t) === 'pending').length;
    const totalRevenue = active.reduce((s, t) => s + t.amount, 0);
    ...
    return { total: allSales.length, pendingCount, revenue: totalRevenue, avg: avgOrder, topItem };
  }, [allSales]);
  ...
  }, [deleting, viewing, deleteTransaction]);
  ...
  }, [updateTransaction, viewing]);

  const handlePaymentComplete = useCallback((updated: Transaction) => {
    updateTransaction(updated);
    setViewing(updated);
    setPayingOrder(null);
  }, [updateTransaction]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {allSales.length} total orders · {allSales.filter((t) => statusOf(t) === 'completed').length} completed
            {kpis.pendingCount > 0 && ` · ${kpis.pendingCount} pending`}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <KpiCard label="Total Orders"   value={String(kpis.total)}          sub={`${kpis.pendingCount} pending · ${allSales.filter((t) => statusOf(t) === 'completed').length} completed`} icon={ShoppingCart} color="bg-amber-500" />
        <KpiCard label="Total Revenue"  value={formatCurrency(kpis.revenue)} sub="Completed orders only" icon={TrendingUp}   color="bg-emerald-500" />
        <KpiCard label="Avg Order"      value={`৳${kpis.avg}`}              sub="Per completed order"   icon={DollarSign}   color="bg-blue-500"    />
        <KpiCard label="Top Item"       value={kpis.topItem ? kpis.topItem[0].split(' ').slice(0, 2).join(' ') : '—'} sub={kpis.topItem ? `${kpis.topItem[1]} sold` : 'No data'} icon={UtensilsCrossed} color="bg-violet-500" />
      </div>
      ...
      {showFilters && (
        <div className="bg-white border border-slate-200 rounded-2xl p-4 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
          {/* Status */}
          <div>
            <label className="block text-[10px] font-bold uppercase tracking-wide text-slate-500 mb-1.5">Status</label>
            <select
              value={statusFilter}
          ...
          {/* Payment */}
          <div>
            <label className="block text-[10px] font-bold uppercase tracking-wide text-slate-500 mb-1.5">Payment</label>
            <select
              value={methodFilter}
          ...
          {/* Channel */}
          <div>
            <label className="block text-[10px] font-bold uppercase tracking-wide text-slate-500 mb-1.5">Channel</label>
            <select
              value={channelFilter}
          ...
          {/* Date From */}
          <div>
            <label className="block text-[10px] font-bold uppercase tracking-wide text-slate-500 mb-1.5">From Date</label>
            <input
              type="date"
              value={dateFrom}
          ...
          {/* Date To */}
          <div>
            <label className="block text-[10px] font-bold uppercase tracking-wide text-slate-500 mb-1.5">To Date</label>
            <input
              type="date"
              value={dateTo}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500">
        <span className="flex items-center gap-1.5">
          <Filter size={12} />
          Showing <strong className="text-slate-700">{filtered.length}</strong> of <strong className="text-slate-700">{allSales.length}</strong> orders
        </span>
        ...
        {paginatedData.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center px-4">
            ...
            <p className="text-sm text-slate-400 mt-1">
              {search || activeFilters > 0 ? 'Try adjusting your search or filters' : 'Orders placed from New Order will appear here'}
            </p>
            {(search || activeFilters > 0) && (
              <button onClick={clearFilters} className="mt-3 text-sm text-amber-600 hover:text-amber-700 font-semibold">
                Clear all filters
              </button>
            )}
          </div>
            ...
            <DataTableHead>
              <DataTableHeadRow>
                ...
                <DataTableHeadCell align="left">
                  <SortBtn field="date" label="Date / Time" sortField={sortField} sortDir={sortDir} onSort={handleSort} />
                </DataTableHeadCell>
                <DataTableHeadCell align="center">
                  <span className="inline-flex justify-center w-full">
                    <SortBtn field="items" label="Items" sortField={sortField} sortDir={sortDir} onSort={handleSort} />
                  </span>
                </DataTableHeadCell>
                ...
                <DataTableHeadCell align="right">
                  <span className="inline-flex justify-end w-full">
                    <SortBtn field="amount" label="Amount" sortField={sortField} sortDir={sortDir} onSort={handleSort} />
                  </span>
                </DataTableHeadCell>
                ...
                return (
                  ...
                  >
                    <DataTableCell align="left">
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <span className={`shrink-0 inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold ${cfg.bg} ${cfg.text}`}>
                            <StatusIcon size={10} />
                            {cfg.label}
                          </span>
                        </div>
                        {tx.customerName && (
                          <p className="text-xs text-slate-400 mt-0.5 truncate">{tx.customerName}</p>
                        )}
                      </div>
                    </DataTableCell>

                    <DataTableCell align="left">
                      <p className="text-xs font-semibold text-slate-700 tabular-nums">{formatDate(tx.date)}</p>
                    ...
                    <DataTableCell align="center">
                      <span className="text-sm font-bold text-slate-700 tabular-nums">{itemCount(tx)}</span>
                      <p className="text-[10px] text-slate-400">items</p>
                    </DataTableCell>

                    <DataTableCell align="center">
                      <span className={`inline-flex items-center justify-center gap-1 text-xs font-semibold ${mCfg.color}`}>
                        <MIcon size={13} />
                        {mCfg.label}
                      </span>
                    </DataTableCell>
                    ...
                    <DataTableCell align="center">
                      {chCfg && ChIcon ? (
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${chCfg.badgeClass}`}>
                          <ChIcon size={10} />
                          {chCfg.label}
                        </span>
                      ) : (
                    ...
                    <DataTableCell align="right">
                      <span className={`text-sm font-bold tabular-nums ${isTop ? 'text-violet-700' : 'text-slate-800'}`}>
                    ...
                    <DataTableCell align="center">
                      <div className="flex items-center justify-center gap-0.5" onClick={(e) => e.stopPropagation()}>
                        {status === 'pending' && (
                          <button
                            type="button"
                            onClick={() => { setViewing(tx); setPayingOrder(tx); }}
      ...
      <ManagerPasswordModal
        ...
        onClose={() => { setShowManagerPin(false); setPendingDelete(null); }}
      ...
      {deleting && (
        <DeleteConfirmModal
          ...
          onCancel={() => { setDeleting(null); setPendingDelete(null); }}
        />
      )}
    </div>
  );
