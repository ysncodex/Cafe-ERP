...
function InlineAddField({ value, onChange, onAdd, onCancel, placeholder }: InlineAddFieldProps) {
  return (
    <div className="flex gap-2">
      <input
        ...
        className="flex-1 h-11 px-4 bg-indigo-50 border border-indigo-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
        autoFocus
      />
      <button
        type="button"
        onClick={onAdd}
        className="h-11 px-4 bg-indigo-600 text-white rounded-xl font-semibold hover:bg-indigo-700 transition-colors flex items-center justify-center"
      >
        Add
      </button>
      <button
        type="button"
        onClick={onCancel}
        className="h-11 px-3 border border-slate-200 rounded-xl text-slate-500 hover:bg-slate-50 transition-colors flex items-center justify-center"
      >
        <X size={16} />
      </button>
    </div>
...
export default function ProductCosts() {
  ...
  const productByMethod = useMemo(() => {
    ...
    productExpenses.forEach((t) => {
      if (t.method in b) b[t.method as keyof typeof b] += t.amount;
    });
    return b;
  }, [productExpenses]);
  ...
  const handleAddItemName = useCallback(() => {
  ...
  }, [newItemName, addItemName, setValue]);
  ...
  const handleAddSupplier = useCallback(() => {
    ...
    try {
      ...
      handleError(error, {
        action: 'add_supplier',
        severity: 'medium',
        metadata: { supplier: trimmed },
      });
    }
  }, [newSupplier, addSupplier, setValue]);
          ...
          <div className="px-5 md:px-6 py-4 bg-slate-50/60 border-b border-slate-200">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2.5">
              ...
              <div className="flex items-center gap-2 sm:ml-auto shrink-0">
                ...
                <ExportDropdown config={exportConfig} disabled={filteredProductExpenses.length === 0} />
              </div>
            </div>
