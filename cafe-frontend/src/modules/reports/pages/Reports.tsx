        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          <section>
            <div className="flex justify-between mb-2">
              ...
              <span className="text-sm font-bold text-orange-600">{formatCurrency(totalProductCost)}</span>
            </div>
            <div className="space-y-1.5">
              {productCosts.map((item) => (
                <div key={item.name} className="flex justify-between text-sm px-3 py-2 bg-orange-50 rounded-lg">
          ...
          <section>
            <div className="flex justify-between mb-2">
              ...
              <span className="text-sm font-bold text-purple-600">{formatCurrency(totalFixedCost)}</span>
            </div>
            <div className="space-y-1.5">
              {fixedCosts.map((item) => (
                <div key={item.name} className="flex justify-between text-sm px-3 py-2 bg-purple-50 rounded-lg">
                  <span className="text-slate-700">{item.name}</span>
                  <span className="font-semibold text-purple-600">{formatCurrency(item.amount)}</span>
                </div>
              ))}
              {fixedCosts.length === 0 && (
...
function SummaryCard({
  ...
  return (
    ...
    >
      <p className={`text-xs font-bold uppercase tracking-wider mb-1 ${isColored ? 'text-white/80' : 'text-slate-500'}`}>
        {label}
      </p>
      <p className={`text-2xl font-extrabold ${isColored ? 'text-white' : 'text-slate-900'}`}>{value}</p>
      {delta && compareLabel && (
        <div className="mt-2 flex items-center gap-2">
          <DeltaBadge change={delta.change} pct={delta.pct} invert={delta.invert} />
          <span className={`text-xs ${isColored ? 'text-white/60' : 'text-slate-400'}`}>vs {compareLabel}</span>
        </div>
      )}
      {onClick && (
        <p className="text-xs text-rose-500 mt-2 font-medium">Tap for breakdown →</p>
      )}
    </Tag>
  );
}
...
const CHANNELS = [
  { key: 'inStore' as const, label: 'In-Store', icon: Store, bar: 'bg-teal-500', text: 'text-teal-700', bg: 'bg-teal-50' },
  { key: 'foodpanda' as const, label: 'Foodpanda', icon: Bike, bar: 'bg-orange-500', text: 'text-orange-700', bg: 'bg-orange-50' },
  { key: 'foodi' as const, label: 'Foodi', icon: Utensils, bar: 'bg-violet-500', text: 'text-violet-700', bg: 'bg-violet-50' },
];

// ─── Main ─────────────────────────────────────────────────────────────────────

export default function Reports() {
  const { stats, setCustomDateRange, filteredTransactions, transactions, customDateRange } = useERP();
  ...
  const comparison = useMemo(() => {
    if (comparisonPeriod === 'none' || !customDateRange.from || !customDateRange.to) return null;
    ...
    const cmpTx = transactions.filter((t) => t.date && t.date >= cmpStart && t.date <= cmpEnd);
    const sales = cmpTx.filter((t) => t.type === 'sale' || t.type === 'sale_adjustment').reduce((s, t) => s + t.amount, 0);
    const expenses = cmpTx.filter((t) => t.type === 'expense_product' || t.type === 'expense_fixed').reduce((s, t) => s + t.amount, 0);
  ...
  const periodLabel = formatPeriodLabel(customDateRange.from, customDateRange.to);
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <SummaryCard
              ...
              delta={comparison ? { change: comparison.salesChange, pct: comparison.salesPct } : undefined}
            ...
            <SummaryCard
              ...
              delta={comparison ? { change: comparison.expensesChange, pct: comparison.expensesPct, invert: true } : undefined}
              compareLabel={comparison?.label}
            />
            <SummaryCard
              ...
              delta={comparison ? { change: comparison.profitChange, pct: comparison.profitPct } : undefined}
              compareLabel={comparison?.label}
            />
            <div className="p-5 rounded-xl border border-slate-200 bg-white shadow-sm">
              <p className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Key Ratios</p>
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-orange-600">{pctOf(stats.totalSales, stats.totalExpenses)}%</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-4 p-4 rounded-xl border border-emerald-200 bg-emerald-50/60">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-emerald-600 mt-0.5">
                  {giftStats.itemCount} gift item{giftStats.itemCount === 1 ? '' : 's'} · retail value {formatCurrency(giftStats.totalValue)}
                </p>
              </div>
            </div>
          )}
              ...
              <div className="px-6 py-5 space-y-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <span className="font-bold text-slate-900">{formatCurrency(stats.totalSales)}</span>
                ...
                <div>
                  ...
                  >
                    ...
                    <div className="text-right">
                      <span className="font-bold text-rose-600">({formatCurrency(stats.totalProductCost)})</span>
                      <span className="text-xs text-rose-400 ml-2">({pctOf(stats.totalSales, stats.totalProductCost)}%)</span>
                    </div>
                  </button>
                  {cogsOpen && stats.topProducts.length > 0 && (
                    <div className="mt-2 ml-5 space-y-1 border-l-2 border-rose-100 pl-3">
                      {stats.topProducts.map((item) => (
                        <div key={item.name} className="flex justify-between text-sm text-slate-600 py-0.5">
                          <span>{item.name}</span>
                          <span className="font-medium text-rose-500">{formatCurrency(item.cost)}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="flex justify-between items-center py-2 px-3 bg-slate-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <span className={`font-bold ${stats.grossProfit >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                      {formatCurrency(stats.grossProfit)}
                    </span>
                    <span className="text-xs text-slate-400 ml-2">({pctOf(stats.totalSales, stats.grossProfit)}%)</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <div className="text-right">
                      <span className="font-bold text-purple-600">({formatCurrency(stats.totalFixedCost)})</span>
                      <span className="text-xs text-purple-400 ml-2">({pctOf(stats.totalSales, stats.totalFixedCost)}%)</span>
                    </div>
                  </button>
                  {opexOpen && stats.topFixed.length > 0 && (
                    <div className="mt-2 ml-5 space-y-1 border-l-2 border-purple-100 pl-3">
                      {stats.topFixed.map((item) => (
                        <div key={item.name} className="flex justify-between text-sm text-slate-600 py-0.5">
                          <span>{item.name}</span>
                          <span className="font-medium text-purple-500">{formatCurrency(item.amount)}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="border-t-2 border-slate-900 pt-4 flex justify-between items-center">
                  <span className={`text-lg font-black uppercase ${stats.profit >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                    {stats.profit >= 0 ? 'Net Profit' : 'Net Loss'}
                  </span>
                  <span className={`text-2xl font-black ${stats.profit >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                  ...
                  return (
                    <div key={key}>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <div className="text-right">
                          <span className="text-sm font-bold text-slate-800">{formatCurrency(ch.revenue)}</span>
                          <span className="text-xs text-slate-400 ml-1">{ch.share.toFixed(0)}%</span>
                        </div>
                      </div>
                      <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${bar}`} style={{ width: `${Math.min(ch.share, 100)}%` }} />
                      </div>
                    </div>
                  );
                })}
          ...
          {trendData.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-xl p-6">
              ...
              <div className="mt-4 space-y-3">
                {trendData.map((day) => (
                  <div key={day.date} className="flex items-center gap-3">
                    <span className="w-14 text-xs font-medium text-slate-500 shrink-0">{day.date}</span>
              ...
              <div className="flex gap-4 mt-4 text-xs text-slate-500">
                ...
                <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-rose-400" /> Expenses</span>
