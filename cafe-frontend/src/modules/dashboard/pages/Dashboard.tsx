...
function countDaysInclusive(from: Date, to: Date) {
  ...
  return Math.round((startOfDay(to).getTime() - startOfDay(from).getTime()) / msPerDay) + 1;
}

/** Human-readable label for the Sales Overview period panel. */
function formatDashboardPeriodLabel(
  ...
  if (customDateRange.from && customDateRange.to) {
    ...
    const to = startOfDay(new Date(customDateRange.to));
    ...
    if (from.getTime() === to.getTime()) {
      if (from.getTime() === today.getTime()) {
        return {
          title: 'Today',
          subtitle: from.toLocaleDateString('en-US', {
            weekday: 'long',
            month: 'long',
            day: 'numeric',
            year: 'numeric',
      ...
      return {
        title: from.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }),
        subtitle: String(from.getFullYear()),
      };
    }
    ...
    const isFullMonth =
      sameMonth && from.getDate() === 1 && to.getTime() === startOfDay(monthEnd).getTime();

    if (isFullMonth) {
      return {
        title: from.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
        subtitle: `Full month · ${dayLabel}`,
      };
    }
    ...
    if (sameMonth) {
      return {
        title: from.toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
        subtitle: `${from.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} – ${to.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} · ${dayLabel}`,
      };
    }
    ...
    if (sameYear) {
      return {
        title: `${from.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} – ${to.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`,
        subtitle: dayLabel,
      };
    }

    return {
      title: `${from.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })} – ${to.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`,
      subtitle: dayLabel,
    };
  }

  const now = new Date();
  const defaults: Record<DateRange, { title: string; subtitle: string }> = {
    today: {
      title: 'Today',
    ...
    prev_month: {
      title: 'Last Month',
      subtitle: new Date(now.getFullYear(), now.getMonth() - 1, 1).toLocaleDateString('en-US', {
        month: 'long',
        year: 'numeric',
      }),
    },
    custom: {
      title: 'Custom Period',
      subtitle: 'Select a date range above',
    },
    all: {
      title: 'All Time',
      subtitle: 'Every recorded transaction',
    },
  };

  return defaults[dateRange] ?? defaults.month;
}
...
function useTodayStats(transactions: ReturnType<typeof useERP>['transactions']) {
  return useMemo(() => {
    ...
    for (const transaction of transactions) {
      ...
      if (transactionDate < todayStart || transactionDate >= tomorrowStart) continue;
...
/** Returns period sales totals (payment methods, channels, profit) from filteredTransactions. */
function usePeriodStats(filteredTransactions: ReturnType<typeof useERP>['filteredTransactions']) {
  return useMemo(() => {
    let cash = 0,
      bkash = 0,
      bank = 0;
    ...
    for (const t of filteredTransactions) {
      if (t.type === 'sale') {
        ...
        if (t.method === 'bkash') bkash += t.amount;
        if (t.method === 'bank') bank += t.amount;
        if (t.channel === 'in_store') inStore += t.amount;
        if (t.channel === 'foodpanda') foodpanda += t.amount;
        if (t.channel === 'foodi') foodi += t.amount;
      }
...
function useInsights(
  period: ReturnType<typeof usePeriodStats>,
...
export default function Dashboard() {
  ...
  const period = usePeriodStats(filteredTransactions);
  const insights = useInsights(period, stats);
  const periodLabel = useMemo(
    () => formatDashboardPeriodLabel(customDateRange, dateRange),
